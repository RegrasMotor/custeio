...
public class Concepto820_COFINS extends JUnitBase {
	private final static String rulesFile = "rules/costeo/BRA/environmentVars/p_cofins.gdst;"+
											...
											"rules/costeo/BRA/Concepto 820 - COFINS.drl";
...
//		ksession.insert(new FactTarifa(34, "Preço Hora Extra"));
		...
		for (DetalleCosteo detalleCosteo : detallesCosteo) {
			...
			} else if (detalleCosteo.getCodigoConcepto()==820 && detalleCosteo.getDescripcion().equals("COFINS")) {
				assertEquals("COFINS", detalleCosteo.getDescripcion());
				assertEquals(20, detalleCosteo.getValor1(), delta);
				assertEquals(0, detalleCosteo.getValor2(), delta);
				assertEquals(0, detalleCosteo.getValor3(), delta);
				assertEquals((17822.06+42.62)*(20/100d), detalleCosteo.getTotal(), delta);				
			} else {
