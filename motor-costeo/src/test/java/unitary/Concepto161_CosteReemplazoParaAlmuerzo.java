...
public class Concepto161_CosteReemplazoParaAlmuerzo extends JUnitBase {
	private final static String rulesFile = "rules/costeo/BRA/environmentVars/v_custo_ria.gdst;" +
    ...
    public void comprobarAccion() {
		...
		for (DetalleCosteo detalleCosteo : detallesCosteo) {
			if (detalleCosteo.getCodigoConcepto()==161) {
				if (detalleCosteo.getCodigoAgrupacion1().equals("VD") && detalleCosteo.getCodigoAgrupacion2().equals("SVIG - MG")) {
					assertEquals("Reposição intervalo almoço - SVIG - MG/VD", detalleCosteo.getDescripcion());
					assertEquals(6, detalleCosteo.getValor1(), delta);
					...
					assertEquals("Reposição intervalo almoço - SVIG - MG/IN V1", detalleCosteo.getDescripcion());
