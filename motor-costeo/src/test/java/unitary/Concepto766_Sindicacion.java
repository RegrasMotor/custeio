...
public class Concepto766_Sindicacion extends JUnitBase {
	private final static String rulesFile = "rules/costeo/BRA/environmentVars/v_sindicancia.gdst;" +
											"rules/costeo/BRA/nivelCosteo.drl;" + 
											"rules/costeo/BRA/Concepto 766 - Sindicacion.drl";
    ...
    public void comprobarAccion() {
		...
		for (DetalleCosteo detalleCosteo : detallesCosteo) {
			if (detalleCosteo.getCodigoConcepto()==766) {
				if (detalleCosteo.getCodigoAgrupacion1().equals("VD") && detalleCosteo.getCodigoAgrupacion2().equals("SVIG - MG")) {
					assertEquals("Valor Sindicancia - SVIG - MG/VD", detalleCosteo.getDescripcion());
					...
					assertEquals(8*8, detalleCosteo.getTotal(), delta);
				} else if (detalleCosteo.getCodigoAgrupacion1().equals("IN V1") && detalleCosteo.getCodigoAgrupacion2().equals("SVIG - MG")) {
					assertEquals("Valor Sindicancia - SVIG - MG/IN V1", detalleCosteo.getDescripcion());
					assertEquals(4, detalleCosteo.getValor1(), delta);
					assertEquals(11, detalleCosteo.getValor2(), delta);
					assertEquals(0.0, detalleCosteo.getValor3(), delta);
					assertEquals(4*11, detalleCosteo.getTotal(), delta);
				} else {
