...
public class Concepto720_Reciclaje extends JUnitBase {
	private final static String rulesFile = "rules/costeo/BRA/environmentVars/v_reciclagem.gdst;" +
											"rules/costeo/BRA/nivelCosteo.drl;" + 
											"rules/costeo/BRA/Concepto 720 - Reciclaje.drl";											
    ...
    public void comprobarAccion() {
		...
		for (DetalleCosteo detalleCosteo : detallesCosteo) {
			if (detalleCosteo.getCodigoConcepto()==720) {		
				if (detalleCosteo.getCodigoAgrupacion1().equals("VD") && detalleCosteo.getCodigoAgrupacion2().equals("SVIG - MG")) {
					assertEquals("Reciclagem - SVIG - MG/VD", detalleCosteo.getDescripcion());
					...
					assertEquals("Reciclagem - SVIG - MG/IN V1", detalleCosteo.getDescripcion());
					assertEquals(4, detalleCosteo.getValor1(), delta);
					assertEquals(10, detalleCosteo.getValor2(), delta);
					assertEquals(0.0, detalleCosteo.getValor3(), delta);
					assertEquals(10*4, detalleCosteo.getTotal(), delta);
				} else {
